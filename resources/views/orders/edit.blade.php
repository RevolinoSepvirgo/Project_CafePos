  ...
  <form method="POST" action="{{ route('orders.update', $order->id) }}">
    ...
    <div class="row">
      ...
      <div class="col-md-8">
        ...
        <div class="mb-3">
          <button type="button" class="btn btn-sm btn-light category-btn active" onclick="filterMenu('all')">Semua</button>
          @php
            $kategoriUnik = $menus->pluck('category.name')->unique()->filter();
          @endphp
          @foreach ($kategoriUnik as $cat)
            <button type="button" class="btn btn-sm btn-light category-btn" onclick="filterMenu('{{ strtolower($cat) }}')">{{ $cat }}</button>
          @endforeach
        </div>
          ...
          <div class="mb-3">
            ...
            <select class="form-select" name="table_id" required>
              ...
              @foreach ($tables as $table)
                <option value="{{ $table->id }}" {{ $order->table_id == $table->id ? 'selected' : '' }}>{{ $table->name }}</option>
              @endforeach
            </select>
          </div>
