                                ...
                                <td>
                                    <form action="{{ route('tables.changeStatus', $table->id) }}" method="POST"
                                        ...
                                        <select name="status" onchange="this.form.submit()"
                                            class="form-select form-select-sm me-2" style="width: 110px;">
                                            <option value="kosong" {{ $table->status == 'kosong' ? 'selected' : '' }}>Kosong
                                            </option>
                                            <option value="terisi" {{ $table->status == 'terisi' ? 'selected' : '' }}>Terisi
                                            </option>
                                            <option value="dipesan" {{ $table->status == 'dipesan' ? 'selected' : '' }}>
                                                Dipesan</option>
                                        </select>
                                        @php
                                    ...
                                    @if (auth()->user()->role === 'admin')
                                        <td class="text-center">
                                            ...
                                            <form action="{{ route('tables.destroy', $table->id) }}" method="POST"
                                                class="d-inline" onsubmit="return confirm('Yakin ingin menghapus meja ini?')">
                                                @csrf
                                                @method('DELETE')
